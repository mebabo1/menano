...
#pragma clang diagnostic pop
    ...
    if (!memType.has_value())
        throw LSFG::vulkan_error(VK_ERROR_UNKNOWN,
            "Unable to find memory type for image");
    ...
    if (res != VK_SUCCESS)
        throw LSFG::vulkan_error(res,
            "Failed to bind memory to Vulkan image");
...
Image::Image(const Core::Device& device, VkExtent2D extent, VkFormat format,
    ...
    if (res != VK_SUCCESS || imageHandle == VK_NULL_HANDLE)
        throw LSFG::vulkan_error(
            res,
            "Failed to create Vulkan image (AHB)");
    ...
    if (res != VK_SUCCESS || memoryHandle == VK_NULL_HANDLE) {
        ...
        throw LSFG::vulkan_error(
            res,
            "Failed to import AHB into VkDeviceMemory");
    }
    ...
    if (res != VK_SUCCESS || viewHandleAhb == VK_NULL_HANDLE) {
        ...
        throw LSFG::vulkan_error(
            res,
            "Failed to create AHB image view");
    }
